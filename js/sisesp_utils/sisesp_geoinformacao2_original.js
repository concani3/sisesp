...
const CLUSTER_SCALE_THRESHOLD = 50000;
...
// cores para os clusters de equipamentos
var vsis_corescluster = ["#45acf2","#aaa"];
//
...
* --------------------------------------------------------------------------
*/
vsis_map.addLayers([vsis_layermapquest, vsis_layerosm, vsis_layerosgeo, vsis_layerstreets,vsis_layersatellite, vsis_layerpopDF,vsis_layerpopGO,vsis_layerregioes, vsis_layerridegroup, vsis_layerRA,
                    vsis_layercenso, vsis_layersetor, vsis_layerquadras, vsis_layerconjunto, vsis_layerlote, vsis_equipamentossaudeDF]);
...
var vsis_estrategiaEquipamentos = [
    [ new OpenLayers.Strategy.BBOX(), new OpenLayers.Strategy.Cluster({distance:50,threshold:1}) ],
    [ new OpenLayers.Strategy.BBOX(), new OpenLayers.Strategy.Cluster({distance:50,threshold:1}) ]
];
var vsis_estrategiaEquipamentos2 = [
            new OpenLayers.Strategy.BBOX(),
            new OpenLayers.Strategy.Cluster({distance:50,threshold:1})
];

var vsis_layerEscolasDF = new OpenLayers.Layer.Vector(MENS_24,{
        visibility: false, // Com isto as features só serão carregadas quando o usuário selecionar o layer
        isBaseLayer:false,
        opacity:1,
        displayInLayerSwitcher:true,
        attribution:MENS_24,
        strategies: vsis_estrategiaEquipamentos[0],
...
var vsis_layerSegurancaDF = new OpenLayers.Layer.Vector(MENS_26,{
        visibility: false, // Com isto as features só serão carregadas quando o usuário selecionar o layer
        isBaseLayer:false,
        opacity:1,
        displayInLayerSwitcher:true,
        attribution:MENS_26,
        strategies: vsis_estrategiaEquipamentos[1],
        protocol: new OpenLayers.Protocol.WFS({
            version: "1.1.0",
            srsName: "EPSG:900913",
            url: HOSTNAME_WFS,
...
var vsis_styleMapCluster = [
        new OpenLayers.StyleMap({
                'default': new OpenLayers.Style({
                    strokeColor:"#E8E40F",
                    fillColor:vsis_corescluster[0],
...
var minhasfeaturesdefault = [
        new OpenLayers.StyleMap({'default': new OpenLayers.Style({fillColor:"#45ACF2"})}) ,
        new OpenLayers.StyleMap({'default': new OpenLayers.Style({fillColor:"#aaaaaa"})})
];
    vsis_layerEscolasDF.styleMap = vsis_styleMapCluster[0];
    vsis_layerSegurancaDF.styleMap = vsis_styleMapCluster[1];
    vsis_map.addLayers([vsis_layerEscolasDF,vsis_layerSegurancaDF]);
    vsis_previousMapScale = vsis_map.getScale();

var minhalista = [MENS_24,MENS_26];
    vsis_map.events.register('zoomend', vsis_map, function() {
        for (var i in minhalista){

            var vsis_layerEquip = this.getLayersByName(minhalista[i])[0];
            console.log("---->   ",minhalista[i],vsis_layerEquip);
            console.log("estilo: ",minhasfeaturesdefault[i]);
            if ((vsis_previousMapScale > CLUSTER_SCALE_THRESHOLD) && (this.getScale() < CLUSTER_SCALE_THRESHOLD)) {
                        ...
                        vsis_layerEquip.styleMap = minhasfeaturesdefault[i];
            }
            if ((vsis_previousMapScale < CLUSTER_SCALE_THRESHOLD) && (this.getScale() > CLUSTER_SCALE_THRESHOLD)) {
                        vsis_layerEquip.strategies[1].activate();
                        vsis_layerEquip.refresh({force: true});
                        vsis_layerEquip.styleMap = vsis_styleMapCluster[i];
            }
        }
